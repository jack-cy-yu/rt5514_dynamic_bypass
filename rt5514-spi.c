...
static int rt5514_pcm_state_change_check(unsigned int val)
{
	unsigned int ret, loop=0, match=0;
	
	rt5514_spi_read_addr(RT5514_PCM_STA_CHECK,&ret);
	ret = (ret >> 16) & 0xff;
	if (ret == val)
		return RT5514_FMT_CHANGE_SUCCESS;
	else {
		while (loop < 30){
			msleep(1);
			rt5514_spi_read_addr(RT5514_PCM_STA_CHECK,&ret);
			...
			pr_info("Wait 1ms for format to be changed!, match:%d\n",match);
		}
		if (match)
			return RT5514_FMT_CHANGE_SUCCESS;
		else
			return RT5514_FMT_CHANGE_FAIL;
	}
}

static int rt5514_check_dsp_status(struct rt5514_dsp *rt5514_dsp)
{
	unsigned int dsp_status;
	int ret;
	/**
	* 0x18002fa8:0x0 Hotword trigger
	* 0x18002fa8:0x1 Get tic ns
	* 0x18002fa8:0x2 bypass DSP
	*/
	rt5514_spi_read_addr(0x18002fa8,&dsp_status);
	if (dsp_status == 0x2){
		if (rt5514_dsp->lrclk == 16000 &&
			rt5514_dsp->pcm_fmt == SNDRV_PCM_FORMAT_S16_LE){
				rt5514_spi_write_addr(0x18001028, 0x80);
				ret = rt5514_pcm_state_change_check(0x80);
				if (ret == RT5514_FMT_CHANGE_FAIL)
					pr_err("Failed to set 16K/S16_LE");
			}
		else if (rt5514_dsp->lrclk == 48000 &&
			rt5514_dsp->pcm_fmt == SNDRV_PCM_FORMAT_S16_LE){
				rt5514_spi_write_addr(0x18001028, 0x81);
				ret = rt5514_pcm_state_change_check(0x81);
				if (ret == RT5514_FMT_CHANGE_FAIL)
					pr_err("Failed to set 48K/S16_LE");
			}
		else if (rt5514_dsp->lrclk == 16000 &&
			rt5514_dsp->pcm_fmt == SNDRV_PCM_FORMAT_S32_LE){
				rt5514_spi_write_addr(0x18001028, 0x90);
				ret = rt5514_pcm_state_change_check(0x90);
				if (ret == RT5514_FMT_CHANGE_FAIL)
					pr_err("Failed to set 16K/S32_LE");
			}
		else if (rt5514_dsp->lrclk == 48000 &&
			rt5514_dsp->pcm_fmt == SNDRV_PCM_FORMAT_S32_LE){
				rt5514_spi_write_addr(0x18001028, 0x91);
				ret = rt5514_pcm_state_change_check(0x91);
				if (ret == RT5514_FMT_CHANGE_FAIL)
					pr_err("Failed to set 48K/S32_LE");
			}
		else
			pr_info("Unsupported clock rate or format!\n");

		return RT5514_DSP_BYPASS;
	}
	else if (dsp_status == 0x1){
		rt5514_spi_write_addr(0x18001028, 0x0);
		return RT5514_DSP_GET_TIC;
	}
	else if (dsp_status == 0x0){
		rt5514_spi_write_addr(0x18001028, 0x0);
		return RT5514_DSP_HOT_TIGGER;
	}
}
...
static void rt5514_spi_copy_work(struct work_struct *work)
{
	...
	if (rt5514_dsp->get_size >= rt5514_dsp->buf_size ||
		rt5514_dsp->dsp_status == RT5514_DSP_BYPASS) {
...
static void rt5514_helper(struct rt5514_dsp *rt5514_dsp)
{
	...
	rt5514_dsp->dsp_status = rt5514_check_dsp_status(rt5514_dsp);
	...
	if ((device_id != RT5514_DEVICE_ID) || (wdg_status)) {
		...
		if (rt5514_dsp->dsp_status == RT5514_DSP_GET_TIC) {
			       ...
			       struct snd_pcm_hw_params *hw_params)
{
	...
	rt5514_dsp->lrclk = params_rate(hw_params);
	rt5514_dsp->pcm_fmt = params_format(hw_params);
	rt5514_dsp->dsp_status = rt5514_check_dsp_status(rt5514_dsp);
	pr_info("lrclk:%d,pcm_fmt:%d\n",rt5514_dsp->lrclk,rt5514_dsp->pcm_fmt);
